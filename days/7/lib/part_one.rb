...
module DaySeven
  module PartOne
    ...
    def self.match_equation(operands, answer)
      ['+', '*'].repeated_permutation(operands.size - 1).any? do |operators|
        answer == solve_equation(operands, operators)
      end
    end

    def self.sum_of_valid_answers(equations)
      equations.reduce(0) { |acc, eq| match_equation(eq[1], eq[0]) ? acc + eq[0] : acc }
    end
